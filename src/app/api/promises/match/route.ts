...
interface MatchRequest {
  politicianId: string
  promiseId?: string // Optional: match specific promise
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { politicianId, promiseId, minConfidence = 0.6 } = body

    if (!politicianId) {
      return NextResponse.json(
        { error: 'Missing required field: politicianId' },
        { status: 400 }
      )
    }
    ...
    // Perform matching
    console.log(
      `Matching ${promises.length} promises against ${actions.length} actions...`
    )
